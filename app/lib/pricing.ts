 ...
 */
export function effectiveTier(tiers: PriceTier[], quantity: number): PriceTier | undefined {
...
export function lineSubtotalCents(tiers: PriceTier[], quantity: number): number | null {
  const tier = effectiveTier(tiers, quantity);
  return tier ? tier.price_cents * quantity : null;
}